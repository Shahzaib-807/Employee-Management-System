...
import { getAllEmployees, addEmployee, updateEmployee, deleteEmployee, setEmployeeStatus, searchEmployees, filterEmployeesByDepartment, filterEmployeesByStatus, getUniqueDepartments, getTotalEmployees, getTotalDepartments, getAverageSalary, getRecentEmployees, sortEmployees, employeesToCsv, validateEmployeeForm, } from "./employeeService.js";
import { getUserByEmployeeId, updateUserCredentials, createEmployeeUser, validateCredentialsForm, validatePasswordOnly, getPasswordStrength, getAllPasswordRequests, getPendingRequestCount, approvePasswordChangeRequest, rejectPasswordChangeRequest, logAction, getRecentAuditLog, } from "./userService.js";
import { setCurrentUser, getUsers } from "./storage.js";
import { showToast, showConfirmModal, setFieldError, clearFieldErrors, formatCurrency, formatDate, formatDateTime, maskPassword, downloadTextFile, escapeHtml, } from "./ui.js";
...
const statRecentEmployees = document.getElementById("stat-recent-employees");
const statAverageSalary = document.getElementById("stat-average-salary");
...
const sortableHeaders = document.querySelectorAll("[data-sort-field]");
const employeeForm = document.getElementById("employee-form");
const formTitle = document.getElementById("form-title");
const submitBtn = document.getElementById("employee-submit-btn");
const cancelEditBtn = document.getElementById("cancel-edit-btn");
const credentialsFieldGroup = document.getElementById("credentials-field-group");
...
const detailsModalRoot = document.getElementById("details-modal-root");
const credentialsModalRoot = document.getElementById("credentials-modal-root");
// Settings view elements
const myEmailInput = document.getElementById("settings-my-email");
const myNewPasswordInput = document.getElementById("settings-my-password");
const myConfirmPasswordInput = document.getElementById("settings-my-confirm-password");
const myCredentialsForm = document.getElementById("my-credentials-form");
const hrAccountsList = document.getElementById("hr-accounts-list");
...
function toggleStatus(id) {
    ...
    logAction(currentUser.name, currentUser.role, "Employee Status Changed", `${employee.fullName} marked as ${newStatus}.`);
...
// ------------------------------------------------------------
function renderPasswordStrengthMeter(password, barEl, labelEl) {
...
function openManageCredentialsModal(employeeId) {
  ...
  `;
    ...
    const toggleBtn = document.getElementById("cred-password-toggle");
    const close = () => {
        credentialsModalRoot.innerHTML = "";
    };
    overlay?.addEventListener("click", (event) => {
    ...
    form.addEventListener("submit", (event) => {
        ...
        logAction(currentUser.name, currentUser.role, "Credentials Updated", `Updated login credentials for ${employee.fullName}.`);
...
function openAssignCredentialsModal(employeeId) {
    const employee = getAllEmployees().find((e) => e.id === employeeId);
    if (!employee)
        return;
    credentialsModalRoot.innerHTML = `
  ...
  `;
    ...
    const form = document.getElementById("assign-credentials-form");
    const emailInput = document.getElementById("assign-email");
    const passwordInput = document.getElementById("assign-password");
    const confirmInput = document.getElementById("assign-confirm-password");
    const strengthBar = document.getElementById("assign-strength-bar");
    const strengthLabel = document.getElementById("assign-strength-label");
    const close = () => {
        credentialsModalRoot.innerHTML = "";
    };
    overlay?.addEventListener("click", (event) => {
    ...
    form.addEventListener("submit", (event) => {
        ...
        if (errors.email)
            setFieldError("assign-email", errors.email);
        if (errors.password)
            setFieldError("assign-password", errors.password);
        if (errors.confirmPassword)
            setFieldError("assign-confirm-password", errors.confirmPassword);
        ...
        logAction(currentUser.name, currentUser.role, "Login Assigned", `Assigned login credentials to ${employee.fullName}.`);
...
function startEditingEmployee(id) {
    const employee = getAllEmployees().find((e) => e.id === id);
    if (!employee)
        return;
    ...
    document.getElementById("salary").value = String(employee.salary);
...
employeeForm.addEventListener("submit", (event) => {
    ...
    const errors = validateEmployeeForm(formInput, isNewEmployee, editingEmployeeId ?? undefined);
    clearFieldErrors(EMPLOYEE_FORM_FIELDS);
    ...
    if (isNewEmployee) {
        ...
        logAction(currentUser.name, currentUser.role, "Employee Added", `Added ${newEmployee.fullName} (${newEmployee.department}) with login access.`);
        ...
        logAction(currentUser.name, currentUser.role, "Employee Updated", `Updated details for ${employeeData.fullName}.`);
...
function confirmDeleteEmployee(id) {
    ...
    if (!employee)
        return;
    showConfirmModal(`Are you sure you want to delete <strong>${escapeHtml(employee.fullName)}</strong>? This action cannot be undone and will also remove their login access.`, () => {
        deleteEmployee(id);
...
function renderDepartments() {
    ...
    departmentsListEl.innerHTML = departments
        ...
        const avgSalary = Math.round(deptEmployees.reduce((sum, e) => sum + e.salary, 0) / deptEmployees.length);
        return `
...
function renderSettings() {
    ...
    // --- HR Accounts ---
    const hrUsers = getUsers().filter((u) => u.role === "hr");
    if (hrUsers.length === 0) {
...
function openEditHrModal(userId) {
    const hrUser = getUsers().find((u) => u.id === userId);
    if (!hrUser)
        return;
    credentialsModalRoot.innerHTML = `
  ...
  `;
    ...
    const form = document.getElementById("hr-edit-form");
    const emailInput = document.getElementById("hr-edit-email");
    const passwordInput = document.getElementById("hr-edit-password");
    const toggleBtn = document.getElementById("hr-edit-password-toggle");
    const close = () => {
        credentialsModalRoot.innerHTML = "";
    };
    overlay?.addEventListener("click", (event) => {
...
pendingRequestsList.addEventListener("click", (event) => {
    ...
    if (revealId) {
        const span = document.querySelector(`[data-request-password="${revealId}"]`);
